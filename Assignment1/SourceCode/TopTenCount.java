...
public class TopTenCount {
	...
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != 3) {
			System.err.println("Usage: TopTenCount <input file path> <output file path1> <output file path2>");
			System.exit(2);
		}

		// job chaining - output of job1 given to input to job2
		// first job - gives pairs and their mutual friends list
		Job getMutualFriendsJob = Job.getInstance(conf1, "TopTenCount");
		getMutualFriendsJob.setJarByClass(TopTenCount.class);
		getMutualFriendsJob.setMapperClass(TopTenCount.Map.class);
		getMutualFriendsJob.setReducerClass(TopTenCount.Reduce.class);
		...
		// Second Job - gives pairs, count of mutual friends , and mutual friends list
		{
			...
			Job job2 = Job.getInstance(conf2, "TopTenCount");
			job2.setJarByClass(TopTenCount.class);
			job2.setMapperClass(TopTenCount.Map2.class);
			job2.setReducerClass(TopTenCount.Reduce2.class);
